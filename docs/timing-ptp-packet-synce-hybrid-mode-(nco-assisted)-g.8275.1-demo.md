# G.8275.1 PTP Boundary Clock with SyncE Hybrid Mode


## 1 Objective

Demonstrate G.8275.1 telecom PTP with SyncE hybrid frequency and phase synchronization.

## 2 Test topology

The topology is GNSS to DUT1 (PCB135/VSC7558), then a SyncE and G.8275.1 PTP link to DUT3 (EVB-LAN969x 24-Cu).
...
```console
(config)# ! PTP instance, G.8275.1 boundary clock
...
`preferred-adj` is not set, auto-select picks **common** for G.8275.1, which selects
...
```console
(config)# ! PTP instance, G.8275.1 boundary clock (acts as slave toward DUT1)
(config)# ptp 0 mode boundary onestep ethernet twoway vid 1 0 profile g8275.1 mep 1
(config)# ptp 0 filter-type aci-basic-phase
...
## 5 How hybrid mode activates on DUT3

Three conditions must all be true for the servo to enter HYBRID mode:

| Condition                                 | Setting                                           |
| ----------------------------------------- | ------------------------------------------------- |
| `CLOCK_OPTION_SYNCE_DPLL` selected        | profile g8275.1 + auto adj-method → common        |
...
```
```{ .text .no-copy }
  Slave port 2   FREQ_LOCKED   (SyncE locked, phase still converging)
...
```
```{ .text .no-copy }
  OffsetFromMaster   -0.000,000,169,914   (converging)
...
# ! Check which port is slave and phase offset from master
